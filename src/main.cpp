...
int width = 35;
int height = 30;
const int socket = OpenFlaschenTaschenSocket("client.ft");  // hostname.
UDPFlaschenTaschen canvas(socket, width, height);

int gap_width = 640/width;
int gap_height = 480/height;
int window;
GLuint texColorBuffer; //Buffer to for RGB
...
void DrawGLScene()
{
    ...
    */
    ...
    int y;
    Color color;
    for(int w=0; w < 640; w+=gap_width){
        for(int h=0; h<480; h = h = h + gap_height){
            glReadPixels(w, h, 1, 1, GL_RGB, GL_FLOAT, &color);
            if(w != 0){
                x = w/gap_width;
            }
            else
...
{
    std::cout << "gl_depth_text" << gl_depth_tex << std::flush << std::endl;
    printf("GL thread\n");
